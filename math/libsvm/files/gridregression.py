...
[additional parameters for svm-train] dataset"""
	...
	out_filename = f'{dataset_title}.out'
	png_filename = f'{dataset_title}.png'
...
class LocalWorker(Worker):
    def run_one(self,c,g,p):
    	cmdline = f'{svmtrain_exe} -s 3 -c {c} -g {g} -p {p} -v {fold} {pass_through_string} {dataset_pathname}'
...
class SSHWorker(Worker):
    def __init__(self,name,job_queue,result_queue,host):
        ...
        self.cwd = os.getcwd()
    def run_one(self,c,g,p):
    	cmdline = f'ssh {self.host} "cd {self.cwd}; {svmtrain_exe} -s 3 -c {c} -g {g} -p {p} -v {fold} {pass_through_string} {dataset_pathname}"'
#	print cmdline
    	result = Popen(cmdline,shell=True,stdout=PIPE).stdout
    	for line in result.readlines():
    		if "Cross" in str(line):
...
def main():
	...
	for _ in range(nr_local_worker):
		...
		for (c,g,p) in line:
			while (c,g,p) not in done_jobs:
				...
				if mse < best_mse:
				    ...
				    best_p = 2.0**p1
				print(
					f"[{worker}] {c1} {g1} {p1} {mse} (best c={best_c}, g={best_g}, p={best_p}, mse={best_mse})"
				)

#            db.append((c,g,r,done_jobs[(c,g,r)]))

	job_queue.put((WorkerStopToken,None,None))
	print(f"{best_c} {best_g} {best_p} {best_mse}")
